...
export default function Rides(props) {
  ...
  if(props.SearchTermTo || props.SearchTermDest){
    changedPost1 = posts.filter(item => item.origin.some(city=>city.toLowerCase().includes(props.SearchTermTo.toLowerCase())));
    changedPost = changedPost1.filter(item => item.destination.some(city=>city.toLowerCase().includes(props.SearchTermDest.toLowerCase())));
  }else if(props.SearchTermTo){
    changedPost = posts.filter(item => item.origin.some(city=>city.toLowerCase().includes(props.SearchTermTo.toLowerCase())));
  }else if(props.SearchTermDest){
    changedPost = posts.filter(item => item.destination.some(city=>city.toLowerCase().includes(props.SearchTermDest.toLowerCase())));
  }else{
    changedPost = posts;
  }
